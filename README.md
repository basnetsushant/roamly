        ...
        <div className="flex gap-x-4">
          {categories.map((item) => {
            ...
            return (
              ...
              >
                <article
                  className={`p-3 flex flex-col items-center cursor-pointer duration-300  hover:text-primary w-25 ${
  ...
  return (
    <article className="group relative">
      <Link href={`/properties/${propertyId}`}>
        <div className="relative h-75 mb-2 overflow-hidden rounded-md">
          <Image
            src={image}
            fill
...
export function SkeletonCard() {
  return (
    <div>
      <Skeleton className="h-75 rounded-md" />
...
import { Skeleton } from '@/components/ui/skeleton';
function loading() {
  return <Skeleton className='h-75 md:h-125 w-full rounded' />;
}
...
export function ChartsLoadingContainer() {
  return <Skeleton className="mt-16 w-full h-75 rounded" />;
}
```
